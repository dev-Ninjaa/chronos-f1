...
class ChronosF1Enhanced {
  ...
  setupUI() {
    ...
    document.getElementById("toggleWeather").addEventListener("click", () => {
      ...
      document.getElementById("weatherSection").style.display = this.showWeather
        ? "block"
        : "none";
    });
  ...
  selectDriver(code, isMultiSelect = false, drivers = null) {
    ...
    const driverMap =
      drivers ||
      (this.currentTelemetry && this.currentTelemetry.frame
        ? this.currentTelemetry.frame.drivers
        : null);
    if (driverMap) {
  ...
  updateAIStatus(data) {
    ...
    if (data.langflow) {
      document.getElementById("strategyInsightsSection").style.display =
        "block";
    }
    ...
    if (data.docling) {
      ...
      console.log(
        `📚 Docling enabled: ${data.regulations_count} regulation sections loaded`,
      );
    }
    ...
    if (debriefSection)
      debriefSection.style.display = data.race_debrief ? "block" : "none";
    const ghostSection = document.getElementById("ghostSection");
    if (ghostSection)
      ghostSection.style.display = data.ghost_engine ? "block" : "none";
  ...
  displayAnalysisResults(results) {
    ...
    if (!results || results.length === 0) {
      ...
      insightsBox.innerHTML =
        '<div class="insight-item">Waiting for analysis data...</div>';
      return;
    }
    ...
    const summary = latestResult.steps?.find(
      (s) => s.action === "format_output",
    )?.output;
    ...
    if (summary) {
      ...
      insightsBox.innerHTML =
        '<div class="insight-item">Processing analysis...</div>';
    }
  }
  ...
  showRaceInfo() {
    ...
    document.getElementById("sessionTotalLaps").textContent =
      this.raceData.totalLaps;
  ...
  updateUI(data) {
    ...
    if (data.weather && this.showWeather) {
      document.getElementById("trackTemp").textContent =
        `${data.weather.trackTemp}°C`;
  ...
  updateTelemetryComparison(data) {
    ...
    this.drawComparisonChart(
      "throttleChart",
      this.comparisonHistory.throttle,
      "throttle",
      0,
      100,
    );
  ...
  updateLeaderboard(drivers) {
    ...
    sortedDrivers.forEach(([code, data]) => {
      ...
      if (this.gapMode === "leader" && data.gapToLeader !== undefined) {
        gapText =
          data.position === 1 ? "-" : `+${data.gapToLeader.toFixed(1)}s`;
      } else if (
        this.gapMode === "interval" &&
        data.intervalGap !== undefined
      ) {
        gapText =
          data.position === 1 ? "-" : `+${data.intervalGap.toFixed(1)}s`;
      }
      ...
      let tyreHealthBar = "";
      if (
        data.tyreHealth !== undefined &&
        typeof data.tyreHealth === "number"
      ) {
      ...
      item.title =
        "Left click: select only this driver. Right click: add/remove for comparison.";
  ...
  drawFrame() {
    ...
    if (
      this.showDrs &&
      this.currentTelemetry.trackData &&
      this.currentTelemetry.trackData.drsZones
    ) {
      this.drawDrsZones();
    }
    ...
    if (
      this.currentTelemetry.trackData &&
      this.currentTelemetry.trackData.finishLine
    ) {
      this.drawFinishLine();
    }
  ...
  drawTrack() {
    ...
    this.trackOffsetX =
      padding -
      xMin * this.trackScale +
      (this.canvas.width - trackWidth * this.trackScale) / 2;
    this.trackOffsetY =
      padding -
      yMin * this.trackScale +
      (this.canvas.height - trackHeight * this.trackScale) / 2;
    // this.trackOffsetY -= 5;
  ...
  getDriverColor(code) {
    ...
    if (
      this.currentTelemetry &&
      this.currentTelemetry.driverColors &&
      this.currentTelemetry.driverColors[code]
    ) {
      console.log(
        `Color for ${code}:`,
        this.currentTelemetry.driverColors[code],
  ...
  seek(progress) {
    ...
    const frameIndex = Math.max(
      0,
      Math.min(progress, (this.raceData.totalFrames || 1) - 1),
    );
...
class TTSCommentary {
  constructor() {
    ...
    this.isSupported = !!(this.synth && window.SpeechSynthesisUtterance);
    this.isUnlocked = false;
    this.noVoicesAvailable = false;
    this.warnedNoVoices = false;
    ...
    if (!this.isSupported) {
      this.enabled = false;
      console.warn(
        "⚠️ Web Speech API not available in this browser/environment",
      );
      return;
    }
    ...
    console.log("🔍 TTS diagnostics:", {
      ...
      initialVoices: this.synth.getVoices().length,
      userAgent: navigator.userAgent,
    });

    // Many browsers gate speech output until a user gesture occurs.
    const unlockHandler = () => {
      this.unlock();
  ...
  unlock() {
    if (!this.isSupported || this.isUnlocked) return;
    try {
      const probe = new SpeechSynthesisUtterance(" ");
      probe.volume = 0;
      this.synth.speak(probe);
      this.synth.cancel();
      this.isUnlocked = true;
      console.log("✅ TTS unlocked by user interaction");
      this.flushPendingSpeech();
    } catch (err) {
      console.warn("⚠️ Could not unlock TTS:", err);
    }
  }

  flushPendingSpeech() {
    if (!this.pendingSpeech || !this.enabled || !this.isSupported) return;
    const { text, mode } = this.pendingSpeech;
    this.pendingSpeech = null;
    this.speak(text, mode, true);
  }

  initVoices() {
    const voices = this.synth.getVoices();
    this.noVoicesAvailable = voices.length === 0;

    if (voices.length === 0) {
      console.warn(
        "⚠️ Web Speech is present but no voices are available yet (or system TTS voices are missing)",
      );
      return;
    }
    ...
    if (englishVoice) {
      ...
      console.log(`✅ TTS Voice: ${englishVoice.name}`);
      return;
    }

    this.voice = voices[0] || null;
    if (this.voice) {
      console.log(`✅ TTS Voice (fallback): ${this.voice.name}`);
    }
  }

  speak(text, mode = "fan", fromQueue = false) {
    if (!this.isSupported || !this.enabled || !this.synth || !text) return;
    if (!this.isUnlocked) {
      this.unlock();
      if (!this.isUnlocked) {
        this.pendingSpeech = { text, mode };
        console.warn(
          "⚠️ TTS blocked until user interaction (click/tap/keypress)",
        );
        return;
      }
    }

    if (!this.voice && this.noVoicesAvailable) {
      this.initVoices();
      if (!this.voice && !this.warnedNoVoices) {
        this.warnedNoVoices = true;
        console.warn(
          "⚠️ No speech voices available. On Linux, install/configure system TTS voices (speech-dispatcher/espeak-ng) or use a browser with bundled voices.",
        );
      }
    }
    ...
    utterance.onend = () => {
      ...
      console.log("✅ TTS utterance finished");
    };
    utterance.onerror = (event) => {
      console.error("❌ TTS utterance error:", event.error || event);
      if (!fromQueue && event && event.error === "not-allowed") {
        this.pendingSpeech = { text, mode };
      }
    };
...
function initializeNewFeatures() {
  ...
  const ttsText = document.getElementById("ttsToggleText");
  if (window.ttsCommentary && !window.ttsCommentary.isSupported) {
    if (ttsBtn) ttsBtn.classList.remove("active");
    if (ttsText) ttsText.textContent = "Not Supported";
  } else if (window.ttsCommentary && window.ttsCommentary.noVoicesAvailable) {
    if (ttsText) ttsText.textContent = "No Voices";
  }
  ...
  if (window.chronos && window.chronos.socket) {
    window.chronos.socket.on("ai_commentary", (data) => {
      ...
      if (window.ttsCommentary && data.text) {
        window.ttsCommentary.speak(
          data.text,
          data.mode || currentCommentaryMode,
        );
      }
    });
...
function toggleTTS() {
  if (window.ttsCommentary) {
    if (!window.ttsCommentary.isSupported) {
      const text = document.getElementById("ttsToggleText");
      if (text) text.textContent = "Not Supported";
      console.warn("⚠️ TTS not supported in this browser/environment");
      return;
    }
    ...
    if (text) {
      if (
        !window.ttsCommentary.voice &&
        window.ttsCommentary.noVoicesAvailable
      ) {
        text.textContent = "No Voices";
      } else {
        text.textContent = enabled ? "Voice On" : "Voice Off";
      }
    }
...
function closeTelemetryComparison() {
  ...
  if (window.chronos) {
    ...
    if (
      window.chronos.currentTelemetry &&
      window.chronos.currentTelemetry.frame
    ) {
      window.chronos.updateLeaderboard(
        window.chronos.currentTelemetry.frame.drivers,
      );
    }
...
function displayRegulationContext(data) {
  ...
  setTimeout(() => {
    const items = regulationsDisplay.querySelectorAll(
